...
resource "aws_iam_user" "eksdeveloper_user" {
  name = "${local.aprefix}-eksdeveloper1"
  path = "/"
  force_destroy = true
...
resource "aws_iam_group_membership" "eksdeveloper" {
  name = "${local.aprefix}-eksdeveloper-group-membership"
  users = [
