...
resource "aws_iam_user" "eksreadonly_user" {
  name = "${local.aprefix}-eksreadonly1"
  path = "/"
  force_destroy = true
...
resource "aws_iam_group_membership" "eksreadonly" {
  name = "${local.aprefix}-eksreadonly-group-membership"
  users = [
